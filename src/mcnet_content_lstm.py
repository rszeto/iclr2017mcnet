'''
MCNet with an LSTM over the content frames
'''
...
class MCNET_CONTENT_LSTM(object):
  def __init__(self, image_size, batch_size=32, c_dim=3,
    ...
    self.x_shape = [batch_size, self.image_size[0], self.image_size[1], K, c_dim]
  ...
  def build_model(self):
    ...
    motion_cell = BasicConvLSTMCell([self.image_size[0]/8, self.image_size[1]/8],
                             [3, 3], 256)
    content_cell = BasicConvLSTMCell([self.image_size[0]/8, self.image_size[1]/8],
                             [3, 3], 256)
    pred = self.forward(self.diff_in, self.x, motion_cell, content_cell)
  ...
  def forward(self, diff_in, x, motion_cell, content_cell):
    # Initial state
    m_state = tf.zeros([self.batch_size, self.image_size[0]/8,
                      self.image_size[1]/8, 512])
    c_state = tf.zeros([self.batch_size, self.image_size[0]/8,
                      self.image_size[1]/8, 512])

    # Motion encoder
    reuse = False
    for t in xrange(self.K-1):
      motion_enc_h, res_m = self.motion_enc(diff_in[:,:,:,t,:], reuse=reuse)
      h_dyn, m_state = motion_cell(motion_enc_h, m_state, scope='motion_lstm', reuse=reuse)
      reuse = True

    # Content encoder
    reuse = False
    for t in xrange(self.K):
      content_enc_h, res_c = self.content_enc(x[:,:,:,t,:], reuse=reuse)
      h_cont, c_state = content_cell(content_enc_h, c_state, scope='content_lstm', reuse=reuse)
      reuse = True
    ...
    for t in xrange(self.T):
      if t == 0:
        ...
        motion_enc_h, res_m = self.motion_enc(diff_in, reuse=True)
        h_dyn, m_state = motion_cell(motion_enc_h, m_state, scope='motion_lstm', reuse=True)
        content_enc_h, res_c = self.content_enc(x, reuse=True)
        h_cont, c_state = content_cell(content_enc_h, c_state, scope='content_lstm', reuse=True)
      ...
      # Set x as last frame to compute motion difference
      if t == 0:
        x = x[:,:,:,-1,:]
      if self.c_dim == 3:
        ...
        xt_gray = inverse_transform(x, self.target_scale)
  ...
  def save(self, sess, checkpoint_dir, step):
    model_name = "MCNET_CONTENT_LSTM.model"
