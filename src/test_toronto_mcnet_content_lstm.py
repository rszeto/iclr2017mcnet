...
from mcnet_content_lstm import MCNET_CONTENT_LSTM
from utils import *
...
def eval(prefix, image_size, K, T, E, gpu, target_scale, dataset_label,
  ...
  with tf.device("/gpu:%d" % gpu[0]):
    ...
    model = MCNET_CONTENT_LSTM(image_size=[image_size, image_size], batch_size=1, K=K,
  ...
  with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
    ...
    quant_dir = "../results/quantitative/MNIST/arch=MCNET_CONTENT_LSTM/data=%s/model=%s/" % (dataset_label, prefix)
    ...
    for i in xrange(video_tensor.shape[1]):
      ...
      savedir = "../results/images/MNIST/arch=MCNET_CONTENT_LSTM/data=%s/model=%s/%04d" % (dataset_label, prefix, i)
...
def main(prefix, image_size, K, T, E, gpu, target_scale, dataset_label):
  ...
  checkpoint_dir = "../models/MCNET_CONTENT_LSTM/"+prefix+"/"
