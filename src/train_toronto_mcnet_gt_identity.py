...
from mcnet_gt_identity import MCNET_GT_IDENTITY
from utils import *
...
def main(lr, batch_size, alpha, beta, image_size, K,
  ...
  gt_identities = np.load('../data/MNIST/%s_identities.npz' % dataset_label)['images']
  gt_identities = get_normalized_identities(gt_identities, target_scale)
  ...
  gt_identities_val = np.load('../data/MNIST/%s_val_identities.npz' % dataset_label)['images']
  gt_identities_val = get_normalized_identities(gt_identities_val, target_scale)
  ...
  identity_input = tf.placeholder(tf.float32, shape=gt_identities.shape[1:])
  train_queue = tf.RandomShuffleQueue(10 * batch_size,
                                      2 * batch_size,
                                      [tf.float32, tf.float32, tf.float32],
                                      shapes=[seq.shape[1:], diff.shape[1:], gt_identities.shape[1:]])
  enqueue_train_op = train_queue.enqueue([seq_input, diff_input, identity_input])
  ...
  val_queue = tf.FIFOQueue(seq_val.shape[0],
                           [tf.float32, tf.float32, tf.float32],
                           shapes=[seq_val.shape[1:], diff_val.shape[1:], gt_identities.shape[1:]])
  enqueue_val_op = val_queue.enqueue_many([seq_val, diff_val, gt_identities_val])
  dequeue_val_op = val_queue.dequeue_many(batch_size)
  val_queue_size = val_queue.size()

  updateD = True
  updateG = True
  prefix = 'MCNET_GT_IDENTITY/%s' % dataset_label
  params_arr = [
  ...
  with tf.device("/gpu:%d"%gpu[0]):
    model = MCNET_GT_IDENTITY(image_size=[image_size,image_size], c_dim=1,
  ...
  with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
    ...
    def fill_train_queue():
      ...
      while not is_queue_closed:
        for i in xrange(seq.shape[0]):
          try:
            sess.run(enqueue_train_op, feed_dict={seq_input: seq[i], diff_input: diff[i], identity_input: gt_identities[i]})
    ...
    for iters in xrange(iters, num_iter):
      ...
      seq_batch, diff_batch, identity_batch = sess.run(dequeue_train_op)
      input_dict = {
        model.diff_in: diff_batch,
        model.xt: seq_batch[:,:,:,K-1],
        model.identity: identity_batch,
      ...
      if np.mod(counter, val_freq) == 1:
        ...
        for batch_num, batchidx_val in mini_batches_val:
          if len(batchidx_val) == batch_size:
            seq_batch_val, diff_batch_val, identity_batch_val = sess.run(dequeue_val_op)

            batch_samples, batch_L = sess.run([model.G, L],
                                              feed_dict={model.diff_in: diff_batch_val,
                                                         model.xt: seq_batch_val[:,:,:,K-1],
                                                         model.identity: identity_batch_val,
