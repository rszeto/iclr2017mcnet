...
class MCNET_GT_IDENTITY(object):
  def __init__(self, image_size, batch_size=32, c_dim=3,
    ...
    self.target_shape = [batch_size, self.image_size[0], self.image_size[1],
                         K+T, c_dim]
    self.identity_shape = [batch_size, self.image_size[0], self.image_size[1], c_dim]
    self.p = float(p)
  ...
  def build_model(self):
    ...
    self.identity = tf.placeholder(tf.float32, self.identity_shape, name='identity')
    
    cell = BasicConvLSTMCell([self.image_size[0]/8, self.image_size[1]/8],
                             [3, 3], 256)
    pred = self.forward(self.diff_in, self.xt, self.identity, cell)
  ...
  def forward(self, diff_in, xt, identity, cell):
    # Initial state
    ...
    for t in xrange(self.T):
      if t == 0:
        ...
        h_id, res_i = self.identity_enc(identity, reuse=False)
        h_tp1 = self.comb_layers(h_dyn, h_cont, h_id, reuse=False)
        res_connect = self.residual(res_m, res_c, res_i, reuse=False)
        ...
        h_id, res_i = self.identity_enc(identity, reuse=True)
        h_tp1 = self.comb_layers(h_dyn, h_cont, h_id, reuse=True)
        res_connect = self.residual(res_m, res_c, res_i, reuse=True)
  ...
  def identity_enc(self, xt, reuse):
    res_in  = []
    conv1_1 = relu(conv2d(xt, output_dim=self.gf_dim, k_h=3, k_w=3,
                          d_h=1, d_w=1, name='identity_conv1_1',reuse=reuse))
    conv1_2 = relu(conv2d(conv1_1, output_dim=self.gf_dim, k_h=3, k_w=3,
                          d_h=1, d_w=1, name='identity_conv1_2',reuse=reuse))
    res_in.append(conv1_2)
    pool1 = MaxPooling(conv1_2, [2,2])

    conv2_1 = relu(conv2d(pool1, output_dim=self.gf_dim*2, k_h=3, k_w=3,
                          d_h=1, d_w=1, name='identity_conv2_1',reuse=reuse))
    conv2_2 = relu(conv2d(conv2_1, output_dim=self.gf_dim*2, k_h=3, k_w=3,
                          d_h=1, d_w=1, name='identity_conv2_2',reuse=reuse))
    res_in.append(conv2_2)
    pool2 = MaxPooling(conv2_2, [2,2])

    conv3_1 = relu(conv2d(pool2, output_dim=self.gf_dim*4, k_h=3, k_w=3,
                          d_h=1, d_w=1, name='identity_conv3_1', reuse=reuse))
    conv3_2 = relu(conv2d(conv3_1, output_dim=self.gf_dim*4, k_h=3, k_w=3,
                          d_h=1, d_w=1, name='identity_conv3_2', reuse=reuse))
    conv3_3 = relu(conv2d(conv3_2, output_dim=self.gf_dim*4, k_h=3, k_w=3,
                          d_h=1, d_w=1, name='identity_conv3_3',reuse=reuse))
    res_in.append(conv3_3)
    pool3   = MaxPooling(conv3_3, [2,2])
    return pool3, res_in

  def comb_layers(self, h_dyn, h_cont, h_id, reuse=False):
    comb1 = relu(conv2d(tf.concat(axis=3,values=[h_dyn, h_cont, h_id]),
  ...
  def residual(self, input_dyn, input_cont, input_id, reuse=False):
    n_layers = len(input_dyn)
    res_out = []
    for l in xrange(n_layers):
      input_ = tf.concat(axis=3,values=[input_dyn[l], input_cont[l], input_id[l]])
