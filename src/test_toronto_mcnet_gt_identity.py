...
def eval(prefix, image_size, K, T, E, gpu, target_scale, dataset_label,
         video_tensor, identities, c_dim, checkpoint_dir, best_model):
  ...
  with tf.device("/gpu:%d" % gpu[0]):
    ...
    model = MCNET_GT_IDENTITY(image_size=[image_size, image_size], batch_size=1, K=K,
  ...
  # Transform identities ahead of time
  identities = get_normalized_identities(identities, target_scale)
  ...
  with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
    ...
    quant_dir = "../results/quantitative/MNIST/arch=MCNET_GT_IDENTITY/data=%s/model=%s/" % (dataset_label, prefix)
    ...
    for i in xrange(video_tensor.shape[1]):
      ...
      savedir = "../results/images/MNIST/arch=MCNET_GT_IDENTITY/data=%s/model=%s/%04d" % (dataset_label, prefix, i)
      ...
      xt = seq_batch[:, :, :, K - 1]
      identity = np.expand_dims(identities[i], axis=0)
...
def main(prefix, image_size, K, T, E, gpu, target_scale, dataset_label):
  ...
  test_identities = np.load('../data/MNIST/%s_test_identities.npz' % dataset_label)['images']
  ...
  long_identities = np.load('../data/MNIST/%s_long_identities.npz' % dataset_label)['images']

  c_dim = 1

  checkpoint_dir = "../models/MCNET_GT_IDENTITY/"+prefix+"/"
  best_model = None # will pick last model

  eval(prefix, image_size, K, T, E, gpu, target_scale, dataset_label + '_long',
       long_video_tensor, long_identities, c_dim, checkpoint_dir, best_model)
  tf.reset_default_graph()
  eval(prefix, image_size, K, T, 0, gpu, target_scale, dataset_label,
       test_video_tensor, test_identities, c_dim, checkpoint_dir, best_model)
  tf.reset_default_graph()
